...
def test_create_question_fails_if_unauthenticated():
    ...
    response = client.post(
        "/api/v1/questions/",
        {"question_text": "Is this working?", "pub_date": timezone.now()},
    )
...
def test_create_question_validation_and_error_contract():
    ...
    response = client.post(
        "/api/v1/questions/",
        {"question_text": "This has no question mark", "pub_date": timezone.now()},
    )
...
def test_is_author_or_read_only_permission():
    """Ensure the custom permission allows safe methods and author edits, but blocks non-author edits."""
    ...
    # 1. Safe method (GET) -> Allowed for anyone (even non-authors)
    request = factory.get('/')
    request.user = non_author
    assert permission.has_object_permission(request, None, question) is True
    
    # 2. Unsafe method (PUT) -> Allowed if request.user == object.author
    request = factory.put('/')
    request.user = author
    assert permission.has_object_permission(request, None, question) is True
    
    # 3. Unsafe method (PUT) -> Blocked if request.user != object.author
    request = factory.put('/')
    request.user = non_author
    assert permission.has_object_permission(request, None, question) is False    

@pytest.mark.django_db
def test_notify_new_question_signal(capsys):
    """Test that the post_save signal fires and prints for new questions, but not updates."""
    
    question = QuestionFactory(question_text="Test Signal Question")
    captured = capsys.readouterr()
    assert "SIGNAL TRIGGERED: A new poll question 'Test Signal Question' was just published!" in captured.out
    
    question.question_text = "Updated text"
    question.save()
    captured = capsys.readouterr()
    assert "SIGNAL TRIGGERED: A new poll question" not in captured.out

@pytest.mark.django_db
def test_notify_delete_question_signal(capsys):
    """Test that the post_delete signal fires and prints when a question is deleted."""
    question = QuestionFactory(question_text="Delete Me")
    capsys.readouterr()
    
    question.delete()
    captured = capsys.readouterr()
    assert "SIGNAL TRIGGERED: A poll question 'Delete Me' was just deleted!" in captured.out
